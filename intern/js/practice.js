(function () {
  ...
  function drawTopics(searchTerm) {
    ...
    const selectedIds = new Set(practiceState.topics.filter((topic) => topic.selected).map((topic) => topic.id));
  ...
  function bindSetupEvents() {
    ...
    InternCore.qs('#selectAllTopicsBtn').addEventListener('click', () => {
      practiceState.topics.forEach((topic) => { topic.selected = true; });
      drawTopics(InternCore.qs('#topicSearchInput').value);
    });
...
renderQuestionScreen();
  ...
  function renderQuestionScreen() {
    ...
    InternCore.qs('#nextQuestionBtn').addEventListener('click', () => {
