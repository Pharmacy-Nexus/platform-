...
async function startDailyChallengeBySubject(subjectId, requestedCount) {
  ...
  function getSubjectAccent(subjectName = '', index = 0) {
    ...
    if (key.includes('pharmacology') || key.includes('فارما')) {
      return { icon: '💊', tag: 'Core', bg: 'linear-gradient(135deg, #0ec5a3 0%, #117f93 52%, #0a2340 100%)' };
    }
    if (key.includes('biochemistry') || key.includes('bio')) {
      return { icon: '🧬', tag: 'Pathways', bg: 'linear-gradient(135deg, #f1a64d 0%, #c76a35 48%, #4c2035 100%)' };
    }
    if (key.includes('pharmaceutics') || key.includes('سيوتكس')) {
      return { icon: '🧪', tag: 'Dosage Forms', bg: 'linear-gradient(135deg, #34c3ff 0%, #3a73ff 50%, #182761 100%)' };
    }
    if (key.includes('chemistry') || key.includes('pharmaceutical chemistry') || key.includes('medicinal') || key.includes('سيوتكال')) {
      return { icon: '⚗️', tag: 'Medicinal', bg: 'linear-gradient(135deg, #8d67ff 0%, #4f46d8 50%, #1c1d59 100%)' };
    }
    if (key.includes('clinical')) {
      return { icon: '🩺', tag: 'Practice', bg: 'linear-gradient(135deg, #ff8f70 0%, #d75b6c 48%, #5b1f48 100%)' };
    }
    const fallback = [
      { icon: '📘', tag: 'Subject', bg: 'linear-gradient(135deg, #14b8a6 0%, #0f766e 48%, #123c55 100%)' },
      { icon: '🔬', tag: 'Science', bg: 'linear-gradient(135deg, #60a5fa 0%, #2563eb 50%, #1e2a78 100%)' },
      { icon: '🧠', tag: 'High Yield', bg: 'linear-gradient(135deg, #f59e0b 0%, #b45309 48%, #4b2a15 100%)' },
      { icon: '📚', tag: 'Review', bg: 'linear-gradient(135deg, #a78bfa 0%, #6d28d9 48%, #31165c 100%)' }
    ];
